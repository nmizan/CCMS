...
'Purpose            : CitiCash With Bearer Summary
'Creation date      : 06-June-2016
'Stored Procedure(s):  
'

Imports System.Data.Common
Imports Microsoft.Practices.EnterpriseLibrary.Data.Sql
Public Class FrmCitiCashWithBearerSumm

#Region "user defined codes"

    Dim _formName As String = "DepositCitiCashWithBearerSummary"
    ...
    Private Sub LoadDataGrid()
        ...
        Try
            ...
            Dim commProc As DbCommand = db.GetStoredProcCommand("CMS_CCash_Bearer_GetDetailList")
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                '-------------------Mizan Work (06-06-2016) ----------------------
                dgView.Item(7, i).Value = dt.Rows(i).Item("REFERENCE_NUMBER").ToString()
                dgView.Item(8, i).Value = dt.Rows(i).Item("TITLE").ToString()
                dgView.Item(9, i).Value = dt.Rows(i).Item("FIRST_NAME").ToString()
                dgView.Item(10, i).Value = dt.Rows(i).Item("MIDDLE_NAME").ToString()
                dgView.Item(11, i).Value = dt.Rows(i).Item("LAST_NAME").ToString()
                dgView.Item(12, i).Value = dt.Rows(i).Item("SSN").ToString()
                dgView.Item(13, i).Value = dt.Rows(i).Item("ID_NUMBER").ToString()
                dgView.Item(14, i).Value = dt.Rows(i).Item("PASSPORT_NUMBER").ToString()
                dgView.Item(15, i).Value = dt.Rows(i).Item("NATIONALITY").ToString()
                dgView.Item(16, i).Value = dt.Rows(i).Item("SOURCE_OF_WEALTH").ToString()
                dgView.Item(17, i).Value = dt.Rows(i).Item("PERSON_ID").ToString()
                dgView.Item(18, i).Value = dt.Rows(i).Item("ADDRESS").ToString()
                dgView.Item(19, i).Value = dt.Rows(i).Item("TPH_NUMBER").ToString()
                '-------------------Mizan Work (06-06-2016) ----------------------
                dgView.Item(20, i).Value = dt.Rows(i).Item("INPUT_BY").ToString()
                dgView.Item(21, i).Value = NullHelper.DateToString(dt.Rows(i).Item("INPUT_DATETIME"))
                dgView.Item(22, i).Value = dt.Rows(i).Item("INPUT_DATETIME")
                dgView.Item(23, i).Value = dt.Rows(i).Item("AUTH_BY").ToString()
                dgView.Item(24, i).Value = NullHelper.DateToString(dt.Rows(i).Item("AUTH_DATETIME"))
            Next
            ...
            If MessageBox.Show("Do you really want to Authorize?", "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                ...
                For i = 0 To dgView.Rows.Count - 1
                    ...
                    If dgView.Rows(i).Cells(0).Value = True Then
                        ...
                        db.AddInParameter(commProc, "@MOD_DATETIME", DbType.DateTime, dgView.Rows(i).Cells(19).Value)
                        ...
                        If result = 0 Then
                            lblToolStripStatus.Text = "Information Authorized Successfully !! "
