...
Public Class FrmRptClearClientWiseChargeSumm

    Dim _formName As String = "ReportClearingChargeClientWiseBACHChargingSummary"
    ...
    Private Sub ExportToXl()
        ...
        Try
            ...
            strSql = "SELECT ACC_NO AS 'Dr. A/C No' ,COUNT(ACC_NO) as 'Cheque Volume', " & _
                                "SUM(CONSOL_CHARGE) as 'Calculated Charge' " & _
                            "FROM V_CLEAR_CHECK_CHARGE WHERE VALUE_TYPE=@VALUE_TYPE AND OPR_DATE>=@OPR_DATE_FROM " & _
                            "AND OPR_DATE<=@OPR_DATE_TO AND INSUFF_FUND=0 " & _
                            "GROUP BY ACC_NO"
            ...
            If cmbValueType.SelectedIndex = 0 Then
                ...
                sheet.Cells(1, 1) = "Client BACH Charging Summary " + Environment.NewLine + _
            ...
            sheet.Cells(i + 3, 1) = "Total Charge Collected"
            ...
            strSql = "SELECT ACC_NO AS 'Dr. A/C No' ,COUNT(ACC_NO) as 'Cheque Volume', " & _
                        "SUM(CONSOL_CHARGE) as 'Calculated Charge' " & _
                        "FROM V_CLEAR_CHECK_CHARGE WHERE VALUE_TYPE=@VALUE_TYPE AND OPR_DATE>=@OPR_DATE_FROM " & _
                        "AND OPR_DATE<=@OPR_DATE_TO AND INSUFF_FUND=1 " & _
                        "GROUP BY ACC_NO"
            ...
            sheet.Range(sheet.Cells(i + 5, 1), sheet.Cells(i + 5, 1)).Font.Underline = True
            ...
            sheet.Range(sheet.Cells(i + 7, 1), sheet.Cells(i + 7, 3)).HorizontalAlignment = Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter
            sheet.Range(sheet.Cells(i + 5, 1), sheet.Cells(i + 7, 3)).EntireRow.Font.Bold = True
            ...
            sheet.Range(sheet.Cells(i + 8, 2), sheet.Cells(i + 8 + dt.Rows.Count, 2)).NumberFormat = "###,###,###,##0"
            sheet.Range(sheet.Cells(i + 8, 3), sheet.Cells(i + 8 + dt.Rows.Count, 3)).NumberFormat = "###,###,###,###.00"

            'sheet.Range(excelRange, Type.Missing).VerticalAlignment = Microsoft.Office.Interop.Excel.XlVAlign.xlVAlignTop


            sheet.Cells(i + 5, 1) = "Uncollected Summary:"
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                sheet.Cells(i + 3 + oi, 1) = "G" + dt.Rows(i)(0).ToString()
            ...
            sheet.Range(sheet.Cells(i + 3 + oi, 1), sheet.Cells(i + 3 + oi, 3)).Borders.LineStyle = Excel.XlLineStyle.xlContinuous
            sheet.Range(sheet.Cells(i + 3 + oi, 1), sheet.Cells(i + 3 + oi, 3)).EntireRow.Font.Bold = True
            sheet.Range(sheet.Cells(i + 3 + oi, 1), sheet.Cells(i + 3 + oi, 3)).Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.LightGray)
            sheet.Range(sheet.Cells(i + 3 + oi, 2), sheet.Cells(i + 3 + oi, 2)).NumberFormat = "###,###,###,##0"
            sheet.Range(sheet.Cells(i + 3 + oi, 3), sheet.Cells(i + 3 + oi, 3)).NumberFormat = "###,###,###,###.00"
            sheet.Cells(i + 3 + oi, 1) = "Total Uncollected Charge"
            'sheet.Cells(i + 3 + oi, 2) = dt.Rows.Count.ToString()

            If i = 0 Then
                sheet.Cells(i + 3 + oi, 2) = "0"
                sheet.Cells(i + 3 + oi, 3) = "0"
            Else
                sheet.Cells(i + 3 + oi, 2) = "=SUM(B" + (3 + oi).ToString() + ":B" + (i + 2 + oi).ToString() + ")"
                sheet.Cells(i + 3 + oi, 3) = "=SUM(C" + (3 + oi).ToString() + ":C" + (i + 2 + oi).ToString() + ")"
            End If

            sheet.Range(sheet.Cells(i + 6 + oi, 1), sheet.Cells(i + 7 + oi, 3)).EntireRow.Font.Bold = True
            sheet.Range(sheet.Cells(i + 6 + oi, 2), sheet.Cells(i + 7 + oi, 3)).Borders.LineStyle = Excel.XlLineStyle.xlContinuous
            sheet.Range(sheet.Cells(i + 6 + oi, 2), sheet.Cells(i + 6 + oi, 3)).HorizontalAlignment = Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter
            sheet.Range(sheet.Cells(i + 6 + oi, 2), sheet.Cells(i + 6 + oi, 3)).Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.LightBlue)
            sheet.Range(sheet.Cells(i + 7 + oi, 2), sheet.Cells(i + 7 + oi, 3)).Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.LightGray)

            sheet.Cells(i + 6 + oi, 2) = "Cheque"
            sheet.Cells(i + 6 + oi, 3) = "Charge"
            sheet.Cells(i + 7 + oi, 1) = "Total Collectable"
            sheet.Cells(i + 7 + oi, 2) = "=B" + (oi - 2).ToString() + "+B" + (i + 3 + oi).ToString()
            sheet.Cells(i + 7 + oi, 3) = "=C" + (oi - 2).ToString() + "+C" + (i + 3 + oi).ToString()
